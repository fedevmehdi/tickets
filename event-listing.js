  function getCurrencySymbol(currencyCode) {
            ...
            async function fetchTicketData(eventId) {
                try {
                    const apiUrl = `http://localhost:3000/api/tickets/event/${eventId}`
            ...
            async function populateEventsDiv() {
                ...
                function calculateTotal() {
                    ...
                    if (logButton) {
                        if (currency) {
                            logButton.textContent = `Checkout Now - ${totalAmount.toFixed(2)}${getCurrencySymbol(currency)}`
                        } else {
                ...
                tickets.forEach(ticket => {
                    ...
                    ticketPrice.textContent = `Price: ${ticket.price} ${getCurrencySymbol(ticket.currency)}`
                    ...
                    quantityDropdown.addEventListener("change", () => {
                        ...
                        if (existingTicket) {
                            ...
                            selectedTickets.push({ ticket: ticket.name, quantity, ticketId: ticket._id, currency: ticket.currency })
                        }
